...
describe('loadOutputs', () => {
    ...
    test('loads specified outputs, conditions and formatters', async () => {
        ...
        const output1ModuleMock = jest.fn().mockResolvedValue({ default: output1ClassMock });
        jest.doMock('app/outputs/output1/Output', output1ModuleMock, { virtual: true });

        const output2ClassMock = jest.fn((formatter, outputConfig, appConfig) => ({ output: 'output2', formatter, outputConfig, appConfig }));
        const output2ModuleMock = jest.fn().mockResolvedValue({ default: output2ClassMock });
        jest.doMock('app/outputs/output2/Output', output2ModuleMock, { virtual: true });

        const condition2ClassMock = jest.fn(() => ({ condition: 'condition2' }));
        const condition2ModuleMock = jest.fn().mockResolvedValue({ default: condition2ClassMock });
        ...
        const noFormatFormatterModuleMock = jest.fn().mockResolvedValue({ default: noFormatFormatterClassMock });
        jest.doMock('app/formatters/NoFormatFormatter', noFormatFormatterModuleMock, { virtual: true });

        const formatter2ClassMock = jest.fn(() => ({ formatter: 'formatter2' }));
        const formatter2ModuleMock = jest.fn().mockResolvedValue({ default: formatter2ClassMock });
        jest.doMock('app/formatters/formatter2', formatter2ModuleMock, { virtual: true });
