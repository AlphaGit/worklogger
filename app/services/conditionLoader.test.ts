...
describe('loadCondition', () => {
    test('loads requested condition module', async () => {
        const conditionClassMock = jest.fn().mockReturnValue({ condition: 'condition1' });
        const moduleMock = jest.fn().mockResolvedValue({ default: conditionClassMock });
    ...
    test('loads default true condition if no condition is specified', async () => {
        const result = await loadCondition({ type: undefined });
